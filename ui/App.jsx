...
function Portfolio({pieces,ready}) {
  ...
  return <section id="work" className="work-section">
    ...
    <div className="portfolio-grid">{visible.map((p,i)=><article className="portfolio-card" key={p.slug}>
      <a className={`poster-stage ${p.orientation}`} href={`/assets/portfolio/${p.slug}.pdf`} aria-label={`Open ${p.title} (PDF)`}><img src={`/assets/portfolio/${p.slug}-1.webp`} alt="" width={p.previewWidth} height={p.previewHeight} loading={i<3?'eager':'lazy'}/><span className="preview-action">View PDF <Arrow/></span></a>
      <div className="poster-caption"><a href={`/assets/portfolio/${p.slug}.pdf`}>{p.title} <Arrow/></a><a className="details-link" href={`/portfolio/${p.slug}/`} aria-label={`Details and sources for ${p.title}`}>Details</a></div>
...
function Human({data,ready}) {
  ...
  const pubs=allPubs?data.publications:data.featured;
  return <>
    <section className="intro"><h1>Research, software<br/>and technical communication.</h1><div className="intro-bottom"><p>We build models, tools and visual explanations<br className="desktop-break"/> for research and engineering teams.</p><a className="contact-link" href={`mailto:${data.site.author.email}`}>Get in touch <Arrow/></a></div></section>
    ...
    <section className="editorial-section" id="publications"><h2>Publications</h2><div className="publication-list">{pubs.map(p=><a className="publication-row" key={p.url} href={p.url}><span className="publication-year">{p.year}</span><div><h3>{p.title}</h3><p>{p.venue}</p></div><Arrow/></a>)}<button className="all-publications" disabled={!ready} onClick={()=>setAllPubs(!allPubs)} aria-expanded={allPubs}>{allPubs?'Fewer publications':`All publications (${data.publications.length})`} <span aria-hidden="true">{allPubs?'−':'+'}</span></button></div></section>
    <section className="editorial-section" id="people"><h2>People</h2><div className="people-list">{data.people.map(p=><a href={p.url} key={p.url}><span>{p.name} <Arrow/></span><span className="affiliation">{p.affiliation}</span></a>)}</div></section>
    {data.posts.length>0&&<section className="editorial-section"><h2>Notes</h2><div>{data.posts.map(p=><p key={p.url}><a href={p.url}>{p.title} <Arrow/></a></p>)}</div></section>}
  </>;
}
...
export default function App({data}) {
  ...
  return <div className="site-shell">
    ...
    <header className="home-header"><a className="brand" href="/" aria-label="Mathscapes home"><img src="/assets/ms_light.png" alt="Mathscapes" width="267" height="48"/></a><nav aria-label="Main navigation"><a href={mode==='human'?'#work':'/?mode=human#work'}>Work</a><a href={mode==='human'?'#people':'/?mode=human#people'}>People</a><a href="#contact">Contact <Arrow/></a></nav></header>
    <main id="main-content" ref={main} tabIndex="-1">{mode==='human'?<Human data={data} ready={ready}/>:<Machine data={data}/>}</main>
    <footer id="contact"><div className="footer-row"><a className="email-link" href={`mailto:${data.site.author.email}`}>{data.site.author.email} <Arrow/></a><div className="social-links"><a href="https://github.com/mathscapes">GitHub <Arrow/></a><a href="https://www.linkedin.com/company/mathscapes-research/">LinkedIn <Arrow/></a></div></div><div className="mode-row"><div role="group" aria-label="Viewing mode" className="mode-toggle"><button disabled={!ready} aria-pressed={mode==='human'} onClick={()=>changeMode('human')}><span aria-hidden="true">○</span> Human</button><button disabled={!ready} aria-pressed={mode==='machine'} onClick={()=>changeMode('machine')}><span aria-hidden="true">⌘</span> Machine</button></div></div><noscript><p className="no-script">Browse the <a href="/research.txt">text research index</a>. Enable JavaScript to filter work and switch viewing modes.</p></noscript></footer>
  </div>;
}
